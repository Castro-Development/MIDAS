...
@Component({
    selector: 'account-ledger-fiscal-card',
    template: `
    <!-- Header Section -->
        ...
        <!-- Balance Cards -->
            <mat-card class="balance-card">
              <div class="balance-window">
                  <div>
                    <h1 class="header2">Account #{{accountLedger.accountNumber}} - {{accountLedger.accountName}}</h1>
                  </div>
                  ...
                  <table class="transaction-table">
                    ...
                    <tbody class="">
                      <tr *ngFor="let entry of paginatedEntries" class="">
                        <td class=" whitespace-nowrap">{{convertTimestamp(entry.date) | date:'shortDate'}}</td>
                        <td class="">{{entry.description}}</td>
                        <td class="text-right">{{entry.debitAmount | currency:'USD':'symbol':'1.2-2'}}</td>
                        <td class="text-right">{{entry.creditAmount | currency:'USD':'symbol':'1.2-2'}}</td>
    ...
    `,
    styleUrl:'./account-ledger-fiscal.component.scss',
})
export class AccountLedgerFiscalCard {
